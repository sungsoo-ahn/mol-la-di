...
### Research Focus
Masked diffusion models for molecule generation, with transformer-based autoregressive baseline.

### Key Components
- `src/models/transformer_ar.py` - Autoregressive transformer for adjacency matrix generation
- `src/data/molecule_dataset.py` - QM9/ZINC250k data loading with adjacency matrix representation
- `src/evaluation/metrics.py` - Molecule generation metrics (Validity, Uniqueness, Novelty, FCD, NSPDK MMD)
- `src/scripts/train.py` - Training pipeline with W&B logging
- `src/scripts/sample.py` - Sampling and evaluation

### Running Experiments

**Full training on QM9:**
```bash
bash scripts/training/train_qm9.sh
```

**Full training on ZINC250k:**
```bash
...
**IMPORTANT: Use the debug config for rapid iteration when developing new features or debugging.**

The debug config (`configs/training/qm9_debug.yaml`) provides:
- Small dataset (128 samples, fits in single batch)
- Smaller model (2 layers, 128 dim)
- Faster training (10 epochs, no W&B)
- Quick feedback loop for testing changes

```bash
# Run debug training (~30 seconds)
bash scripts/training/train_debug.sh

# Or directly:
uv run python src/scripts/train.py configs/training/qm9_debug.yaml
```

To enable debug mode in any config, add:
```yaml
data:
  debug: true
  debug_samples: 128  # Number of samples to use
```
...
- **NSPDK MMD**: Graph kernel-based distribution distance

### Expected Outcomes
- QM9: Validity > 90%, Uniqueness > 90%
- ZINC250k: Validity > 80%, Uniqueness > 95%
